  ...
  init: function () {
    ...
    let getLyric = (id) => {
      return new Promise((resolve, reject) => {
        fetch("https://pl-fe.cn/cloud-music-api/lyric?id=" + id)
          .then((res) => {
            return res.json();
          })
          .then((data) => {
    ...
    function loadcplayer(cplayer) {
      ...
      if (!cplayer.prototype.add163)
        cplayer.prototype.add163 = async function add163(id) {
          if (!id) throw new Error("Unable Property.");
          let lyric = await getLyric(id);
          // console.log(lyric)

          return fetch("https://pl-fe.cn/cloud-music-api/song/detail?ids=" + id)
            .then(function (res) {
              return res.json();
            })
            .then(
    ...
    if (
      typeof window.cplayer === "undefined" &&
      !document.getElementById("cplayer-script")
    ) {
